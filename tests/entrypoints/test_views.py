...
class TestBuildingViews:
    async def test_get_buildings(self, async_app, building):
        await building(id=1)
    ...
    async def test_get_building(self, async_app, building):
        await building(id=1)
        async with async_app as app:
    ...
    async def test_update_building(self, session, async_app, building):
        ...
        assert response.status_code == 200
        assert response.json() == {"id": 1}
        building = await BuildingsRepository(session).get(1)
    ...
    async def test_delete_building(self, async_app, building):
        await building(id=1)
        async with async_app as app:
